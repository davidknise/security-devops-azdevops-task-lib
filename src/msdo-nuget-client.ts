...
function getLatestEnviromentVariable(packageName: string, isPreRelease: boolean): string {
    ...
    return `GDN_${packageName.replace(/\./g, '').replace('-', '')}${suffix}`.toUpperCase();
}
...
async function resolveVersion(
    ...
    if (common.isLatest(packageVersion)) {
        let serviceResponse: FindServiceResponse = findService(serviceIndex, 'SearchQueryService', ['3.0.0-beta']);
        let serviceOptions = {
            packageName: packageName,
            packageVersion: packageVersion
        };
...
async function _resolveVersion(
    ...
    let searchQueryServiceUrlWithQuery = `${service['@id']}?q=${packageName}&prerelease=${common.isLatestPreRelease(packageVersion)}`;
    ...
    for (let packageInfo of result['data']) {
 ...
 */
function resolveRequestOptions(accessToken: string = null): Object {
    let options = {
        method: 'GET',
        timeout: 2500,
        headers: {
